...
EXPOSE 8080
CMD ["bash","-lc","JDBC_URL=$(echo \"$DATABASE_URL\" | sed -E 's#^postgres://#jdbc:postgresql://#; s#//[^/@]+@#//#'); echo Using JDBC_URL=$JDBC_URL; export SPRING_DATASOURCE_URL=\"$JDBC_URL\"; exec java $JAVA_OPTS -jar /app/app.jar --server.port=$PORT"]
